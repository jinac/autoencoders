...
def convnet(input_img, filter_param=64, max_filter=256, bias=True):
    """
    """
    img_dim = input_img.shape[1:]
    num_layers = math.frexp(int(img_dim[0]))[1] - 4

    out = bn_conv_layer(input_img, int(img_dim[-1]), 4, 2, use_bias=bias)

    for n in xrange(num_layers):
        num_filters = min(filter_param * (2 ** n), max_filter)
        out = bn_conv_layer(out, num_filters, 4, 2, use_bias=bias)
...
def deconvnet(input_dense, output_dim,
    ...
    for n in xrange(num_layers):
        ...
        out = bn_deconv_layer(out, num_filters, 4, 2, use_bias=bias)
